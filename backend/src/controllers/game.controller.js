...
const InformationController = require('./info.controller');
...
class GameController {
  ...
  async create(req, res) {
    try {
      ...
      if (gameDBData.data.length < 6) {
        ...
        // setup monitoring
        StateController.getInstance().monitor(this.organisation_id, newGameDBData.data.object_id);

        // Return the game
        return res
          .status(201)
          .send(response('Origin New Game Board Created successfully', newGameDBData.data, true));
      }
      // look for completed game to reset and join as owner
      const game = gameDBData.data.find((x) => x.status === 2);

      // Check if game is found
      if (!game) {
        return res.status(400).send(response('No free boards right now', null, false));
      }

      // Reset the game
      await this.GameRepo.update(game._id, {
        owner: {
          user_id,
          user_name,
          image_url,
  ...
  async join(req, res) {
    try {
      ...
      if (
        ...
        if (gameDBData.data.opponent) {
          if (gameDBData.data.opponent.user_id !== user_id) {
            return res.status(400).send(response('opponent already exists', null, false));
          }
        }
      }
      ...
      if (!gameDBData.data.opponent) {
        ...
        const sidebar_update_payload_opponent = await InformationController.sideBarInfo(
          this.organisation_id,
          user_id,
        );

        const sidebar_update_payload_owner = await InformationController.sideBarInfo(
          this.organisation_id,
  ...
  async getById(req, res) {
    try {
      ...
      if (fetchedGame.data !== null) {
        ...
        res.status(200).send(response('Game retrieved successfully', fetchedGame.data));
      } else {
  ...
  async pieceMove(req, res) {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .send(response('Only players are allowed to make moves', null, false));
      }

      // push new move into moves array
      const { moves } = gameDBData.data;
      moves.push({
        user_id,
        position_fen,
        board_state,
  ...
  async capturedPiece(req, res) {
    try {
      ...
      // push new move into moves array
      const { captured_pieces } = gameDBData.data;
      ...
      await centrifugoController.publish(game_id, payload);
      return res.status(200).send(response('chess piece captured', updated, true));
    } catch (error) {
  ...
  async addSpectator(req, res) {
    try {
      ...
      const { spectators } = gameDBData.data;
      ...
      // Return the game
      return res.status(200).send(response('Joined as spectator successful', updated));
    } catch (error) {
  ...
  async removeSpectator(req, res) {
    try {
      ...
      // Return the game
      return res.status(200).send(response('spectator removed successfully', updated));
    } catch (error) {
  ...
  async endGame(req, res) {
    try {
      ...
      if (!gameDBData.data) {
        return res.status(400).send(response('Game does not exist', null, false));
      }
      ...
      await this.GameRepo.update(gameDBData.data._id, {
        is_owner_winner,
        status,
      });
      ...
      StateController.getInstance().stopMonitoring(this.organisation_id, game_id);
  ...
  async resign(req, res) {
    ...
    try {
      ...
      if (isGameExist.data.status === 2) {
        return res.status(400).send(response('Game already ended.', null, false));
      }
      ...
      if (user_id === isGameExist.data.owner.user_id) {
        ...
        return res
          .status(400)
          .send(response('You are not a participant of this game.', null, false));
      }
  ...
  async delete(req, res) {
    try {
      ...
      if (!game.data) {
        return res.status(404).send(response('No such game found in the database', {}, false));
      }

      await this.GameRepo.delete(game.data._id, game.data);

      return res.status(204).send(response('game deleted successfully', {}, false));
    } catch (error) {
