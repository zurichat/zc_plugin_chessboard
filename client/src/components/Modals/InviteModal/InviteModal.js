...
const InviteModal = () => {
  ...
  const [noUser, setNoUser] = useState(false)
    ...
    const handleInput = (e) => {
      ...
      obj = tempPlayersDetails.filter(player => player.name.toLowerCase().includes(e.target.value.toLowerCase()))
      if (obj.length === 0) {
        setNoUser(true);
      } else {
        setNoUser(false)
        ...
        <div className='invite-player__backdrop'>
         <div className="invite-player__modal">
           ...
           <form className="invite-player__form">
            <input type="text" autoComplete="off" name="text" onChange={handleInput} placeholder="Search User..."/>
          </form>

          <div className="invite-player__players">
            {!noUser && <InviteModalPlayer players={playersDetails}/>}
            {noUser && <p className="invite-player__no-user">No player found..</p>}
          </div>
         </div>
        </div>
    )
